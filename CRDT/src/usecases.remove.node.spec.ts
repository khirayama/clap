      ...
      describe(`共同編集者選択範囲が${nodePatterns.b}`, () => {
        describe('編集者と共同編集者が同じ選択範囲の場合', () => {
          it('複数項目が削除され、編集者と共同編集者の選択範囲が次項目に移動していること', () => {
            ...
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
              doc.users[member.id].ids = [doc.document.nodes[1].id, doc.document.nodes[2].id, doc.document.nodes[3].id];
              doc.users[member.id].range = null;
            });
            userDoc.merge(memberDoc);
          ...
          it('複数項目が削除され、編集者と共同編集者の選択範囲が前項目に移動していること', () => {
            ...
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
              doc.users[member.id].ids = [doc.document.nodes[2].id, doc.document.nodes[3].id, doc.document.nodes[4].id];
              doc.users[member.id].range = null;
            });
            userDoc.merge(memberDoc);
        ...
        describe('編集者選択範囲前に共同編集者選択範囲がある場合', () => {
          it('複数項目が削除され、編集者の選択範囲が次項目に移動し、共同編集者選択項目に影響がないこと', () => {
            userDoc.change((doc) => {
              doc.users[user.id].ids = [doc.document.nodes[2].id, doc.document.nodes[3].id];
              doc.users[user.id].range = null;
            });
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
              doc.users[member.id].ids = [doc.document.nodes[0].id, doc.document.nodes[1].id];
              doc.users[member.id].range = null;
            });
            userDoc.merge(memberDoc);
        ...
        describe('編集者選択範囲内に共同編集者選択範囲がある場合', () => {
          it('複数項目が削除され、編集者と共同編集者の選択範囲が次項目に移動していること', () => {
            ...
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
              doc.users[member.id].ids = [doc.document.nodes[2].id, doc.document.nodes[3].id];
              doc.users[member.id].range = null;
            });
            userDoc.merge(memberDoc);
          ...
          it('複数項目が削除され、編集者と共同編集者の選択範囲が前項目に移動していること', () => {
            ...
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
              doc.users[member.id].ids = [doc.document.nodes[3].id, doc.document.nodes[4].id];
              doc.users[member.id].range = null;
            });
            userDoc.merge(memberDoc);
          ...
          it('複数項目が削除され、編集者と共同編集者の選択範囲が親項目に移動していること', () => {
            ...
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
              doc.users[member.id].ids = [doc.document.nodes[0].nodes[0].nodes[1].id];
              doc.users[member.id].range = null;
            });
            userDoc.merge(memberDoc);
        ...
        describe('編集者選択範囲と共同編集者選択範囲に一部重複がある場合', () => {
          it('複数項目が削除され、編集者の選択範囲が次項目に移動し、共同編集者選択項目の重複項目が削除されていること', () => {
            ...
            nodes[4].prev = nodes[0].id;
            expectedDoc.doc.users[user.id].ids = [nodes[4].id];
            expectedDoc.doc.users[member.id].ids = [nodes[0].id];
            nodes.splice(1, 3);
          ...
          it('複数項目が削除され、編集者の選択範囲が前項目に移動し、共同編集者選択項目の重複項目が削除されていること', () => {
            ...
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
              doc.users[member.id].ids = [doc.document.nodes[1].id, doc.document.nodes[2].id];
              doc.users[member.id].range = null;
            });
            userDoc.merge(memberDoc);
        ...
        describe('編集者選択範囲後に共同編集者選択範囲がある場合', () => {
          it('複数項目が削除され、編集者の選択範囲が次項目に移動し、共同編集者選択項目に影響がないこと', () => {
            userDoc.change((doc) => {
              doc.users[user.id].ids = [doc.document.nodes[0].id, doc.document.nodes[1].id, doc.document.nodes[2].id];
              doc.users[user.id].range = null;
            });
            memberDoc.merge(userDoc);
            memberDoc.change((doc) => {
